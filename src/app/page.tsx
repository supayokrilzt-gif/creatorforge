...
function getPersonLabel(question: string) {
  ...
  const patterns = [
    /^คนชื่อ\s*([ก-๙A-Za-z]{1,24})/i,

    /^([ก-๙A-Za-z]{1,24})\s*(?:เขา|เค้า)/i,

    /^([ก-๙A-Za-z]{1,24})\s*(?:คิด|รู้สึก|ชอบ|รัก|มอง|สนใจ|จะโทร|จะทัก|จะติดต่อ|จะกลับ)/i,
  ];
...
function getLoveCrushDirectAnswer(
  tone: Tone,
  question: string
) {
  const focus =
    getLoveQuestionFocus(
      question
    );
  ...
  /*
    สิ่งนี้เป็นการตีความแนวโน้มจากคำถามและไพ่
    ไม่ใช่การยืนยันความคิดส่วนตัวของอีกฝ่าย
  */

  if (focus === "thought") {

    if (tone === "positive") {
      return pickLoveVariant(
        question,
        [
          `ถ้ามองจากภาพรวม ${person}อาจกำลังมองคุณในแง่ค่อนข้างดี มีความสนใจและยังนึกถึงเรื่องระหว่างคุณอยู่ เพียงแต่เขายังเหมือนกำลังดูจังหวะว่าจะขยับเข้ามาใกล้แค่ไหน`,

          `ภาพที่ออกมาค่อนข้างอบอุ่นนะ ${person}มีแนวโน้มว่ายังให้พื้นที่กับคุณอยู่ในความคิด และอาจกำลังชั่งใจว่าถ้าเข้าใกล้อีกนิด ความสัมพันธ์จะเดินไปทางไหน`,

          `เรื่องของคุณยังไม่ได้หลุดออกจากความสนใจของ${person}เสียทีเดียว มีแนวโน้มว่าเขายังคิดถึงความสัมพันธ์นี้ในด้านที่ดี เพียงแต่ยังไม่ได้รีบเปิดไพ่ทั้งหมดในมือของตัวเอง`,
        ],
        11
      );
    }
    ...
    if (tone === "caution") {
      return pickLoveVariant(
        question,
        [
          `ตอนนี้ภาพของ${person}ออกไปทางคิดเยอะและลังเลมากกว่า เขาอาจมีเรื่องของคุณอยู่ในใจ แต่ยังมีบางอย่างทำให้ไม่กล้าตัดสินใจหรือแสดงท่าทีตรง ๆ`,

          `ถ้าถามว่า${person}คิดยังไง ภาพที่เห็นไม่ใช่การปิดใจเสียทีเดียว แต่เหมือนเขากำลังสับสนกับความรู้สึกของตัวเอง จึงมีทั้งจังหวะอยากเข้าใกล้และจังหวะถอยออกมา`,

          `${person}อาจยังนึกถึงคุณอยู่ แต่ตอนนี้ความลังเลมีน้ำหนักมาก ความคิดของเขาเหมือนยังแกว่งไปมา จึงไม่ควรรีบตีความความเงียบว่าเป็นคำตอบสุดท้าย`,
        ],
        23
      );
    }
    ...
    return pickLoveVariant(
      question,
      [
        `${person}มีแนวโน้มว่ายังสนใจและสังเกตท่าทีของคุณอยู่ แต่ภาพยังอยู่ตรงกลาง คือมีบางอย่างดึงให้เขาคิดถึงเรื่องนี้ ขณะเดียวกันก็ยังไม่พร้อมทำให้ชัด`,

        `ภาพรวมเหมือน${person}ยังเก็บเรื่องของคุณไว้คิดอยู่บ้าง เขาไม่ได้ดูปิดสนิท แต่ก็ยังไม่ถึงจุดที่ความคิดนั้นกลายเป็นการตัดสินใจชัด ๆ`,

        `ถ้ามองเฉพาะแนวโน้ม ${person}ยังไม่ได้วางเรื่องนี้ลงทั้งหมด มีทั้งความสนใจ ความสงสัย และการดูเชิงปนกันอยู่`,
      ],
      37
    );
  }
  ...
  if (
    ...
    if (tone === "positive") {
      return pickLoveVariant(
        question,
        [
          `ความรู้สึกระหว่างคุณกับ${person}มีด้านที่ค่อนข้างดี เขาอาจรู้สึกสบายใจหรือดึงดูดกับคุณมากกว่าที่แสดงออก เพียงแต่ยังต้องดูว่าความรู้สึกนี้จะถูกแสดงผ่านการกระทำไหม`,

          `ภาพความรู้สึกออกมาในทางเปิดรับ ${person}มีแนวโน้มว่าสนใจคุณจริงอยู่พอสมควร แต่ความชัดเจนจะอยู่ที่ว่าเขาจะเริ่มเข้าหาและรักษาการติดต่อด้วยตัวเองหรือไม่`,

          `มีพื้นที่ของความรู้สึกดี ๆ อยู่ระหว่างคุณกับ${person} แต่ยังไม่ควรรีบเรียกมันว่าความรักเต็มตัว สิ่งที่น่าสนใจคือเขามีแนวโน้มเปิดรับคุณมากกว่าปิดประตู`,
        ],
        43
      );
    }
    ...
    if (tone === "caution") {
      return pickLoveVariant(
        question,
        [
          `ความรู้สึกของ${person}ตอนนี้ดูไม่ใช่เส้นตรง มีความสนใจอยู่ แต่ก็มีความลังเลหรือความกังวลบางอย่างปนอยู่ด้วย จึงอาจแสดงออกไม่สม่ำเสมอ`,

          `ภาพนี้ไม่ได้บอกว่า${person}ไม่รู้สึกอะไร แต่ความรู้สึกของเขายังไม่นิ่งพอ มีทั้งด้านที่อยากเข้าใกล้และด้านที่อยากรักษาระยะ`,

          `ตอนนี้ยังไม่ควรรีบสรุปว่า${person}ชอบหรือไม่ชอบแบบเด็ดขาด เพราะภาพที่ออกมาคือความรู้สึกที่ยังแกว่งและต้องใช้เวลา`,
        ],
        59
      );
    }
    ...
    return pickLoveVariant(
      question,
      [
        `${person}อาจมีความรู้สึกบางอย่างกับคุณมากกว่าความเฉย ๆ แต่ตอนนี้ยังเป็นความรู้สึกที่กำลังก่อตัวและดูเชิงกันอยู่`,

        `ภาพรวมมีแรงดึงดูดหรือความสนใจอยู่บ้าง แต่${person}ยังไม่ได้แสดงให้เห็นมากพอว่าจะพัฒนาไปไกลแค่ไหน`,

        `ความสัมพันธ์นี้ยังมีพื้นที่ให้ความรู้สึกเติบโต ${person}ดูไม่ได้ปิดรับคุณ แต่ความชัดเจนยังต้องรอจากการกระทำจริง`,
      ],
      71
    );
  }
  ...
  if (focus === "behavior") {
    ...
    if (tone === "caution") {
      return pickLoveVariant(
        question,
        [
          `พฤติกรรมของ${person}ดูคล้ายคนที่กำลังถอยไปจัดการความคิดของตัวเองมากกว่าตัดคุณออกไปทันที ความเงียบจึงอาจมาจากความลังเล ไม่ใช่คำตอบสุดท้าย`,

          `${person}อาจกำลังรักษาระยะเพราะยังไม่แน่ใจว่าจะจัดการความสัมพันธ์นี้อย่างไร จึงเกิดพฤติกรรมใกล้บ้างห่างบ้าง`,

          `สิ่งที่${person}ทำตอนนี้มีลักษณะของการชะลอและดูสถานการณ์ เขาอาจยังมีเรื่องในใจที่ไม่ได้พูด แต่เราไม่ควรเดาแทนทั้งหมดจากความเงียบ`,
        ],
        83
      );
    }
    ...
    return pickLoveVariant(
      question,
      [
        `${person}ยังมีแนวโน้มอยากรักษาการเชื่อมต่อไว้ เพียงแต่รูปแบบการแสดงออกของเขาอาจไม่ตรงหรือเร็วเท่าที่คุณคาด`,

        `พฤติกรรมของ${person}ยังไม่เหมือนคนที่ปิดประตู เขาดูเหมือนกำลังเลือกจังหวะและดูว่าคุณตอบรับอย่างไร`,

        `${person}ยังมีท่าทีของความสนใจอยู่บ้าง แต่ตอนนี้เขาเหมือนต้องการพื้นที่ในการตัดสินใจว่าจะเดินเข้ามาใกล้กว่านี้หรือไม่`,
      ],
      97
    );
  }
  ...
  return tone === "caution"
    ? `เรื่องนี้ยังมีความไม่แน่ใจอยู่พอสมควร ระหว่างคุณกับ${person}อาจยังมีความสนใจหรือความผูกพันบางอย่าง แต่ตอนนี้ควรให้การกระทำจริงเป็นคำตอบมากกว่าพยายามอ่านใจเขา`
    : `ภาพรวมระหว่างคุณกับ${person}ยังมีพื้นที่ให้พัฒนา เขาดูไม่ได้ปิดรับความสัมพันธ์นี้ แต่สิ่งที่จะทำให้คำตอบชัดคือการกระทำที่ต่อเนื่อง ไม่ใช่สัญญาณเพียงครั้งเดียว`;
}
...
function getBaseReading(intent: Intent, tone: Tone, question = ""): Reading {
  if (intent === "love_crush") {
    ...
    const example = pickLoveVariant(
      question,
      focus === "thought"
        ? [
            `ที่น่าดูต่อคือ ${person}เป็นฝ่ายกลับมาเริ่มบทสนทนาเองบ้างไหม จำรายละเอียดเล็ก ๆ ของคุณได้หรือเปล่า หรือพยายามต่อบทสนทนาให้นานขึ้น สิ่งเหล่านี้จะช่วยให้ภาพที่กำลังคลุมเครือชัดขึ้น`,
            `ลองสังเกตช่วงที่ไม่ได้มีเหตุจำเป็นต้องคุยกัน ถ้า${person}ยังหาเรื่องกลับมาคุย ถามถึงชีวิตคุณ หรือจำเรื่องที่คุณเคยเล่าได้ นั่นเป็นสัญญาณที่มีน้ำหนักกว่าการเดาจากข้อความครั้งเดียว`,
            `อย่าเพิ่งจับคำพูดประโยคเดียวมาขยายความ ลองมองเป็นช่วงเวลา ว่า${person}ยังกลับมาเชื่อมต่อกับคุณเองสม่ำเสมอแค่ไหน`,
          ]
        : [
            `ลองดูว่า${person}เป็นฝ่ายเข้าหาคุณเองบ้างไหม สนใจรายละเอียดที่เกี่ยวกับคุณหรือเปล่า และเวลาคุยกันเขาพยายามรักษาบทสนทนาไว้หรือไม่`,
            `ความรู้สึกจะเห็นชัดขึ้นจากพฤติกรรมเล็ก ๆ ที่เกิดซ้ำ เช่น การทักเอง การหาเวลาให้ หรือการใส่ใจเรื่องที่ไม่จำเป็นต้องจำ`,
            `ถ้าความสนใจมีน้ำหนักจริง มักจะไม่ได้ปรากฏแค่ครั้งเดียว แต่จะค่อย ๆ เห็นจากความสม่ำเสมอในการเข้าหาและการรักษาความสัมพันธ์`,
          ],
      131
    );

    const guidance = pickLoveVariant(
      question,
      [
        `ตอนนี้ไม่จำเป็นต้องรีบเค้นคำตอบจาก${person} ลองให้พื้นที่ความสัมพันธ์เดินอีกนิด และรักษาจังหวะของตัวเองไว้ด้วย คุณควรรู้สึกได้ถึงความพยายามจากอีกฝ่าย ไม่ใช่ต้องเป็นคนตีความทุกอย่างอยู่คนเดียว`,
        `สิ่งที่ดีที่สุดตอนนี้คือเปิดพื้นที่ให้เขาแสดงออก แต่ไม่ต้องไล่ตามความชัดเจนจนเหนื่อย ดูว่าความสัมพันธ์มีการขยับจากทั้งสองฝ่ายหรือไม่`,
        `คุณคุยและแสดงความรู้สึกได้อย่างเป็นธรรมชาติ แต่ไม่จำเป็นต้องวางหัวใจทั้งหมดไว้กับการเดาว่าเขาคิดอะไร ให้การกระทำต่อจากนี้ช่วยตอบคำถามแทน`,
      ],
      149
    );

    const hook = pickLoveVariant(
      question,
      [
        `ส่วนที่ยังน่าสนใจคือ ความรู้สึกหรือความคิดนี้จะค่อย ๆ เปลี่ยนเป็นการเข้าหาที่ชัดขึ้น หรือจะยังค้างอยู่ในระยะดูใจกันต่อไป`,
        `สิ่งที่ยังไม่เปิดออกทั้งหมดคือ หลังจากนี้${person}จะเริ่มแสดงออกมากขึ้นหรือไม่ เพราะตรงนั้นจะเป็นตัวแยกระหว่างความสนใจกับความสัมพันธ์ที่พร้อมเดินหน้า`,
        `อีกจุดที่ยังต้องดูคือ จังหวะต่อไปของ${person}จะเป็นการเข้ามาใกล้ขึ้น หรือเพียงรักษาความสัมพันธ์ให้อยู่ในระยะเดิม`,
      ],
      167
    );
      ...
      focus === "thought"
        ? positivePath
          ? pickLoveVariant(
              question,
              [
                `เมื่อมองภาพรวมทั้งสามใบ เรื่องของคุณยังมีพื้นที่อยู่ในความสนใจของ${person} เขาอาจมองคุณในด้านที่ดีและยังคิดถึงความเป็นไปได้ของความสัมพันธ์นี้อยู่ เพียงแต่ยังไม่ได้รีบตัดสินใจ`,
                `ภาพรวมลึกขึ้นแล้ว ความคิดของ${person}ดูไม่ได้ปิดจากคุณ เขายังมีแนวโน้มให้ความสนใจอยู่ แต่สิ่งที่ค้างคือเขาจะกล้าเปลี่ยนความคิดนั้นเป็นการกระทำหรือไม่`,
              ],
              211
            )
          : pickLoveVariant(
              question,
              [
                `พอมองลึกลงไป ${person}ยังมีเรื่องของคุณอยู่ในความคิด แต่ความลังเลเด่นมาก เขาอาจคิดวนหรือไม่แน่ใจว่าควรเดินเข้ามาใกล้หรือรักษาระยะ`,
                `ภาพทั้งสามใบทำให้เห็นความซับซ้อนมากขึ้น ${person}อาจยังนึกถึงคุณ แต่ยังมีบางอย่างทำให้เขาชะลอและไม่อยากตัดสินใจเร็ว`,
              ],
              223
            )
        ...
        ? pickLoveVariant(
            question,
            [
              `พอมองทั้งสามใบ ความรู้สึกระหว่างคุณกับ${person}ยังมีด้านที่ดีอยู่ เขามีแนวโน้มเปิดรับและสนใจคุณ แต่ความสัมพันธ์ยังต้องการการกระทำที่ชัดเจนกว่านี้`,
              `ภาพรวมไม่ได้ออกมาเย็นชาเลย ${person}อาจมีความรู้สึกดีและแรงดึงดูดบางอย่างกับคุณ เพียงแต่ความรู้สึกนั้นยังอยู่ในช่วงค่อย ๆ แสดงตัว`,
            ],
            239
          )
        : pickLoveVariant(
            question,
            [
              `ภาพลึกลงมาแล้ว ความรู้สึกของ${person}ยังมีความไม่แน่นอนอยู่มาก มีทั้งด้านที่อยากรักษาความสัมพันธ์และด้านที่ยังลังเล`,
              `${person}ดูไม่ได้ไม่มีความรู้สึก แต่ภาพรวมยังแกว่ง จึงควรให้เวลาและพฤติกรรมจริงช่วยยืนยันมากกว่าพยายามหาคำตอบจากความเงียบ`,
            ],
            251
          );

    const middle = pickLoveVariant(
      question,
      [
        `สิ่งที่เด่นคือความสัมพันธ์นี้ยังไม่เหมือนเรื่องที่จบสนิท แต่ความชัดเจนจะเกิดก็ต่อเมื่อ${person}เริ่มทำอะไรบางอย่างให้เห็นจริง ไม่ว่าจะเป็นการกลับมาคุยเอง การหาเวลา หรือการรักษาการติดต่ออย่างต่อเนื่อง`,

        `จุดสำคัญไม่ได้อยู่ที่ว่าเขามีความคิดหรือความรู้สึกหรือไม่เพียงอย่างเดียว แต่อยู่ที่สิ่งเหล่านั้นจะกลายเป็นพฤติกรรมจริงหรือเปล่า ถ้าเขาเริ่มขยับเข้าหาด้วยตัวเอง ภาพจะชัดขึ้นมาก`,

        `ตอนนี้ยังเหมือนมีเส้นเชื่อมระหว่างคุณสองคนอยู่ แต่คุณไม่จำเป็นต้องเป็นฝ่ายดึงเส้นนั้นคนเดียว ให้ดูว่า${person}จะช่วยรักษาความต่อเนื่องของความสัมพันธ์ด้วยหรือไม่`,
      ],
      269
    );

    const advice = pickLoveVariant(
      question,
      [
        `ช่วงนี้คุยกับเขาได้อย่างเป็นธรรมชาติ ไม่ต้องทำให้ทุกบทสนทนากลายเป็นการถามหาสถานะ และในขณะเดียวกันก็อย่าละเลยความสบายใจของตัวเอง คุณควรได้รับความชัดเจนจากการกระทำเช่นกัน`,

        `อย่าเร่งให้ความสัมพันธ์ต้องมีชื่อเรียกทันที แต่ก็ไม่จำเป็นต้องรอแบบไม่มีขอบเขต ลองให้เวลาอีกระยะแล้วดูว่าเขาเริ่มเข้าหาคุณเองมากขึ้นหรือไม่`,

        `รักษาความเป็นตัวเองไว้ และปล่อยให้${person}มีพื้นที่แสดงความตั้งใจของเขา ถ้าความสัมพันธ์จะเดินหน้า คุณควรเห็นความพยายามจากทั้งสองด้าน`,
      ],
      283
    );

    const blessing = pickLoveVariant(
      question,
      [
        `ขอให้สิ่งที่ยังคลุมเครือค่อย ๆ เปิดออกในแบบที่ทำให้คุณสบายใจ และให้คนที่รู้สึกกับคุณจริงกล้าแสดงออกด้วยการกระทำ`,

        `ขอให้ความสัมพันธ์นี้พาคุณไปสู่ความชัดเจน ไม่ว่าจะลงเอยทางไหน และให้คุณได้อยู่กับคนที่ไม่ทำให้ต้องเดาความรู้สึกอยู่คนเดียว`,

        `ขอให้คำตอบต่อจากนี้มาจากสิ่งที่คุณเห็นและสัมผัสได้จริง และให้หัวใจของคุณได้รับทั้งความอ่อนโยนและความชัดเจน`,
      ],
      307
    );
...
export default function Home() {
  ...
  async function unlockDeepReading() {
    ...
    /* MEMBER CHECK V21 START */

    // ไพ่ใบแรกใช้ฟรีได้โดยไม่ต้องสมัคร
    // แต่เมื่อต้องการเปิดไพ่ต่อ ต้องเป็นสมาชิกก่อน
    if (!user) {
      setMessage(
        "สมัครสมาชิกฟรีก่อน เพื่อเปิดไพ่ต่อและดูคำทำนายแบบละเอียด"
      );
    ...
    <main className="page">
      ...
      <style jsx global>{`
        ...
        */
        ...
        /* RESULT FIXED V10 START */

        /*
          รอบนี้ไม่อิง parent ของไพ่อีกแล้ว
          ข้อความล็อกกับ viewport โดยตรง
        */
        ...
        */
        ...
        img:not(.deep-cards img) {
          ...
          height: min(78vh, 670px) !important;
        ...
        */
        ...
        strong {
          ...
          text-shadow:
            ...
            0 0 13px rgba(255,207,82,.42) !important;
        }
